...
for time_period in range(time_min, time_max, period_step):
    for start_time in rand_starts:
      ...
      hold_value = ( close[end_time] - close[start_time] ) / close[start_time]
      buy_price = 0
      sell_price = 0
      MACD_value = 0
      own_stock = False

	    #Use MACD method over time-period, long only method
      macd = MACD(close)
      for t in range(start_time, end_time):
        if macd[t] > 0 and macd[t-1] < 0: #Crosses 0 on way up:
          ...
          sell_price = close[t]
          own_stock = False
          MACD_value = macd[t]*(1 + (sell_price - buy_price)/buy_price)
...
#We now have MACD returns vs holdings as a function of time_period
